...
/**
 * Scores class is defined for maintaining a set of scores as 
 * GameEntry objects, which is for storing high scores in an 
...
public class Scores {
	 ...
	 * 
	 * @param {int} index - the range of given index should be between  
	 * 						0 to actual number of game entries minus 1.
	 * 						
	 * @exception IndexOutOfBoundsException - if the given index is not in
	 * 										  the range
	 *
		...
		throws IndexOutOfBoundsException
	{
		if ((index<0) || (index>=nEntries)) {
			throw new IndexOutOfBoundsException ("Invalid index" + Integer.toString(index));
		}
